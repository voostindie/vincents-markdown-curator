...
@Singleton
final class BookProducer
    extends EntityTransformer<Document, Book>
{
    ...
    @Override
    protected Class<Book> targetClass()
    {
        return Book.class;
    }

    @Override
    protected Optional<Book> transform(Document document)
    {
        if (!document.folder().isRoot()
            && document.folder().parent().isRoot()
            && document.folder().name().contentEquals(BOOK_FOLDER))
        {
            var parser = new BookParser();
            document.accept(parser);
            return Optional.of(parser.book);
        }
